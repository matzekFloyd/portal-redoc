...
function mockLocation(initUrl) {
  ...
  return {
      ...
      assign: (newUrl) => url = new URL(newUrl),
      toString: () => url.toString()
  };
}

// @ts-ignore
global.location = mockLocation('https://www.kompany.com/kycapi');
// @ts-ignore
global.fetch = () => new Promise((resolve, reject) => resolve({text: () => null}));
...
// @ts-ignore
import { createStore, loadAndBundleSpec, Redoc } from '@frontend/redoc';
...
const BUNDLES_DIR = dirname(require.resolve('@frontend/redoc'));
